...
const arabicNavLabels: Record<string, string> = {
  "/": "Ø§Ù„Ø±Ø¦ÙŠØ³ÙŠØ©",
  "/upcoming-events": "Ø§Ù„ÙØ¹Ø§Ù„ÙŠØ§Øª Ø§Ù„Ù‚Ø§Ø¯Ù…Ø©",
  "/previous-events": "ÙØ¹Ø§Ù„ÙŠØ§Øª Ø³Ø§Ø¨Ù‚Ø©",
  "/why-us": "Ù„Ù…Ø§Ø°Ø§ Ù†Ø­Ù†ØŸ",
...
function readAuthCta() {
  ...
  try {
    ...
    const role = user?.role_code || user?.role?.code || user?.role
    const href = ["admin", "organizer", "employee", "back_office"].includes(role) ? "/admin" : "/customer"
    return { href, isLoggedIn: true }
  } catch {
    return { href: "/admin", isLoggedIn: true }
  }
}
