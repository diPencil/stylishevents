    ...
    <div className="space-y-6">
      <header className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
        <div>
          ...
          <h1 className="text-2xl font-extrabold tracking-tight text-[#17172f] md:text-3xl">
          ...
          <p className="mt-2 max-w-3xl text-sm font-semibold leading-6 text-slate-500 md:text-base">
            {adminT(language, "overview.subtitle") || "Live operations from MySQL events, bookings, payments, and attendees."}
          </p>
        </div>
      ...
      <section className="grid gap-4 2xl:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-6 min-w-0">
          <div className="grid gap-4 xl:grid-cols-[340px_minmax(0,1fr)]">
            <Card className="rounded-[28px] border-0 bg-white shadow-[0_16px_35px_rgba(93,58,138,0.07)]">
              <CardHeader className="pb-2">
                <CardTitle className="text-base font-extrabold">{adminT(language, "overview.ticketSales") || "Ticket Sales"}</CardTitle>
              </CardHeader>
              <CardContent>
                <div
                ...
                >
                  <div className="flex h-full w-full flex-col items-center justify-center rounded-full bg-white">
                    ...
                    <p className="text-2xl font-extrabold">{formatNumber(computed.ticketsSold)}</p>
                  </div>
                </div>
            ...
            <RevenueTrendCard
              ...
              subtitle={language === "ar" ? "إيرادات الحجوزات المدفوعة من قاعدة البيانات." : "Paid booking revenue from MySQL."}
          ...
          <div className="grid gap-4 lg:grid-cols-2 lg:items-stretch 2xl:min-h-[310px]">
            <InfoGroupCard
          ...
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1.5fr)_minmax(280px,0.9fr)] xl:items-stretch">
                  ...
                  return (
                    <Link key={item.id || item.registration_number} href={`/admin/orders/${item.id || ""}`} className="grid gap-3 rounded-2xl bg-[#f8fbff] p-3 transition hover:bg-[hsl(var(--primary)/0.08)] md:grid-cols-[150px_1fr_120px_100px] md:items-center">
                      ...
                      <div className="min-w-0">
                        <p className="truncate text-sm font-extrabold">{item.doctor_name || item.customer_name || adminT(language, "common.customer") || "Customer"}</p>
                        <p className="truncate text-xs font-bold text-slate-400">{eventTitle(item, language)}</p>
                      </div>
                ...
                {computed.latestReviews.length ? computed.latestReviews.map((review) => (
                  <Link key={review.id} href={`/admin/reviews/${review.id}`} className="grid gap-3 rounded-2xl bg-[#f8fbff] p-3 transition hover:bg-[hsl(var(--primary)/0.08)] sm:grid-cols-[1fr_auto] sm:items-center">
                    <div className="min-w-0">
                      ...
                      <p className="truncate text-xs font-bold text-slate-400">{eventTitle(review, language) || review.comment || "Event feedback"}</p>
                    </div>
          ...
          <Card className="rounded-[28px] border-0 bg-white shadow-[0_16px_35px_rgba(93,58,138,0.07)]">
            <CardHeader className="flex flex-row items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-xs font-bold text-slate-400">{language === "ar" ? "كروت الفعاليات المنشورة والقادمة." : "Published upcoming event cards pulled from MySQL."}</p>
              </div>
              <Button asChild variant="ghost" className="h-9 rounded-2xl px-3 text-xs font-extrabold text-[hsl(var(--primary))]">
        ...
        <aside className="space-y-4 min-w-0">
          <Card className="overflow-hidden rounded-[28px] border-0 bg-white shadow-[0_16px_35px_rgba(93,58,138,0.07)]">
            <CardHeader className="pb-3">
              <CardTitle className="text-base font-extrabold">{adminT(language, "overview.nextEvent") || "Next Event"}</CardTitle>
            </CardHeader>
            <CardContent>
            ...
            <h3 className="mt-4 text-base font-extrabold leading-6">{eventTitle(computed.nextEvent, language) || (language === "ar" ? "لا توجد فعالية قادمة" : "No upcoming event")}</h3>
            <p className="mt-2 text-sm font-medium leading-6 text-slate-400">{eventVenue(computed.nextEvent) || (language === "ar" ? "لم يتم تحديد المكان" : "Venue not set")}</p>
          ...
          <div className="grid gap-3 sm:grid-cols-3 2xl:min-h-[310px] 2xl:auto-rows-fr 2xl:grid-cols-1">
          ...
          <Card className="flex flex-col rounded-[28px] border-0 bg-white shadow-[0_16px_35px_rgba(93,58,138,0.07)] 2xl:min-h-[360px]">
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between gap-3">
                <CardTitle className="text-base font-extrabold">{adminT(language, "overview.eventCalendar") || "Event Calendar"}</CardTitle>
          ...
          <Card className="rounded-[28px] border-0 bg-white shadow-[0_16px_35px_rgba(93,58,138,0.07)]">
            <CardHeader className="pb-2">
              <CardTitle className="text-base font-extrabold">{adminT(language, "overview.liveAttendance") || "Live Attendance"}</CardTitle>
          ...
          <Card className="rounded-[28px] border-0 bg-white shadow-[0_16px_35px_rgba(93,58,138,0.07)]">
            <CardHeader className="pb-2">
              <CardTitle className="text-base font-extrabold">{adminT(language, "overview.quickActions") || "Quick Actions"}</CardTitle>
              <p className="mt-1 text-xs font-bold text-slate-400">{adminT(language, "overview.fastPaths") || "Fast paths for daily admin work."}</p>
            </CardHeader>
            <CardContent className="space-y-2">
              <QuickAction href="/admin/events/create" label={adminT(language, "common.createEvent") || "Create Event"} />
              <QuickAction href="/admin/orders" label={adminT(language, "overview.reviewPayments") || "Review payments"} />
              <QuickAction href="/admin/checkin" label={adminT(language, "overview.openCheckin") || "Open check-in"} />
              <QuickAction href="/admin/certificates/builder" label={adminT(language, "overview.certificateBuilder") || "Certificate builder"} />
            </CardContent>
          </Card>
        </aside>
      </section>
    ...
    <Card className="rounded-[26px] border-0 bg-white shadow-[0_16px_35px_rgba(93,58,138,0.07)]">
      <CardContent className="flex min-h-[104px] items-center gap-4 p-4">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 text-xl font-extrabold text-[#17172f]">{typeof value === "number" ? formatNumber(value) : value}</p>
        </div>
      </CardContent>
    </Card>
  )
...
function InfoGroupCard({ title, subtitle, items }: { title: string; subtitle: string; items: Array<{ icon: any; label: string; value: number }> }) {
  return (
    <Card className="flex h-full min-h-[310px] flex-col rounded-[28px] border-0 bg-white shadow-[0_16px_35px_rgba(93,58,138,0.07)]">
      <CardHeader className="pb-2">
      ...
      <CardContent className="grid flex-1 gap-3 sm:grid-cols-2">
        {items.map((item) => <MiniMetric key={item.label} label={item.label} value={item.value} icon={item.icon} />)}
      </CardContent>
    </Card>
  )
}
    ...
    <Card className="h-full rounded-[24px] border-0 bg-white shadow-[0_14px_30px_rgba(93,58,138,0.06)]">
      <CardContent className="flex h-full items-center gap-3 p-4">
        ...
        <div className="min-w-0">
          ...
          <p className="mt-1 truncate text-lg font-extrabold text-[#17172f]">{typeof value === "number" ? formatNumber(value) : value}</p>
...
function EventCard({ event, language }: { event: any; language: string }) {
  ...
  return (
    <Link href={`/admin/events/${event.id}`} className="group overflow-hidden rounded-[22px] border border-slate-100 bg-[#f8fbff] p-3 transition hover:-translate-y-0.5 hover:shadow-[0_16px_35px_rgba(15,23,42,0.10)]">
      ...
      <div className="pt-3">
        ...
        <div className="mt-3 flex items-end justify-between gap-3">
          ...
          <div className="min-w-[82px] text-end">
            ...
            <p className="text-xs font-black text-[#17172f]">{formatNumber(registrations)} / {formatNumber(capacity)}</p>
          </div>
        </div>
    ...
    <Card className="overflow-hidden rounded-[30px] border-0 bg-white shadow-[0_18px_45px_rgba(93,58,138,0.08)]">
      ...
      <CardContent className="pt-3">
        ...
        <div className="relative" onMouseLeave={() => setActiveIndex(null)}>
          <svg viewBox={`0 0 ${chartWidth} ${chartHeight}`} className="h-[250px] w-full overflow-visible" role="img" aria-label={title}>
            <defs>
...
function QuickAction({ href, label }: { href: string; label: string }) {
  return (
    <Button asChild variant="ghost" className="h-11 w-full justify-between rounded-2xl bg-[#f8fbff] px-4 text-sm font-extrabold text-slate-600 hover:bg-[hsl(var(--primary)/0.10)] hover:text-[hsl(var(--primary))]">
      <Link href={href}>
        <span className="flex items-center gap-2"><LayoutDashboard className="h-4 w-4" />{label}</span>
